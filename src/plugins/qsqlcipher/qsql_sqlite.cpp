...
/*
   ...
   just file names.
*/
bool QSQLiteDriver::open(const QString & db, const QString & user, const QString & password, const QString &, int, const QString &conOpts)
{
    Q_D(QSQLiteDriver);
    if (isOpen())
        close();
    ...
    if (res == SQLITE_OK) {
        ...
        const int ret = sqlite3_key(d->access, password.toUtf8().constData(), password.size());
        if(ret != SQLITE_OK) {
            sqlite3_close(d->access);
            d->access = nullptr;
            setLastError(qMakeError(d->access, tr("Error decrypt database"), QSqlError::ConnectionError, ret));
            setOpenError(true);
            return false;
        }
